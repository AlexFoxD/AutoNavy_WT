...
M8a independent review efa5d44: SPEC PASS, QUALITY PASS, no blocking findings. Reviewer verified real metrics/input/capture session integration, bounded traceback/rotation/throttle, preview freshness/cleanup and finite paired arithmetic plus fresh menu/battle workloads. All six JSON reports agree with CSV and recomputed summaries; final code hashes match. Worker 440-pass/1-excluded log has no warning output. No tests rerun. Historical legacy attribution depends on earlier source audit; live backend behavior/latency and M8b remain outside this gate.

M8b independent review d6b03a6: SPEC PASS, QUALITY PASS, no findings. Verified offline defaults, exact PS5.1/PS7 argv/exit propagation, metadata-only readiness, non-acquiring explicit vJoy diagnostics, checked build paths, actual frozen worker/resource integration and relative/absolute config semantics. Reviewer inspected actual build3/ZIP evidence, 463 full passes, 454 fresh core passes and 26 final focused core passes without rerunning suites/compile. Operation docs and incident disclosure accepted. Hardware, external/Linux CI and M9 whole-branch verification are outside this gate. M8 is complete.
